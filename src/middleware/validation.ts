...
export const validateMyEventRequest = [
  ...
  body("numberOfParticipants")
    .isInt({ min: 1 })
    .withMessage("Number of participants must be a number greater than 0"),
  body("facilities")
    .isArray()
  ...
  body("eventItems.*.name")
    .notEmpty()
    .withMessage("EventItem name is required"),
  body("eventItems.*.price")
    .isFloat({ min: 0 })
    .withMessage("EventItem price is required and must be a positive number"),
  handleValidationErrors,
];
